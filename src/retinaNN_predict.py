...
#sys.pathは現在の検索パスを構成しているディレクトリ名のリストだ（皆さんの環境では違うものが表示されるだろう。これはOSや、Pythonのバージョン、Pythonがインストールされた場所に依存する）。Pythonはこれらのディレクトリを（この順番で）見ていき、インポートしようとしている名前にマッチする.pyファイルを探す。
#③	実を言うと、今述べたことは正しくない。真相はもっと複雑で、すべてのモジュールが.py ファイルの形で格納されているとは限らないのだ。たとえばsysのようないくつかのモジュールは、組み込みモジュールというものであり、これらは実際にはPython自体に埋め込まれている。組み込みモジュールは通常のモジュールと同じように振る舞うが、このモジュールのPythonのソースコードを手に入れることはできない。組み込みモジュールはPythonでは書かれていないのだ！（sysモジュールはc言語で書かれている）
